...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImVec2              WorkOffsetMin;          // Work Area: Offset from centerPos to top-left corner of Work Area. Generally (0,0) or (0,+main_menu_bar_height). Work Area is Full Area but without menu-bars/status-bars (so WorkArea always fit inside centerPos/Size!)
    ImVec2              WorkOffsetMax;          // Work Area: Offset from centerPos+Size to bottom-right corner of Work Area. Generally (0,0) or (0,-status_bar_height).
    ...
    ImVec2  CalcWorkRectPos(const ImVec2& off_min) const                            { return ImVec2(centerPos.x + off_min.x, centerPos.y + off_min.y); }
    ImVec2  CalcWorkRectSize(const ImVec2& off_min, const ImVec2& off_max) const    { return ImVec2(ImMax(0.0f, Size.x - off_min.x + off_max.x), ImMax(0.0f, Size.y - off_min.y + off_max.y)); }
    void    UpdateWorkRect()            { WorkPos = CalcWorkRectPos(WorkOffsetMin); WorkSize = CalcWorkRectSize(WorkOffsetMin, WorkOffsetMax); } // Update public fields

    // Helpers to retrieve ImRect (we don't need to store BuildWorkRect as every access tend to change it, hence the code asymmetry)
    ImRect  GetMainRect() const         { return ImRect(centerPos.x, centerPos.y, centerPos.x + Size.x, centerPos.y + Size.y); }
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  centerPos;                                // Position (always rounded-up to nearest pixel)
...
public:
    ...
    ImRect      Rect() const            { return ImRect(centerPos.x, centerPos.y, centerPos.x + Size.x, centerPos.y + Size.y); }
    float       CalcFontSize() const    { ImGuiContext& g = *GImGui; float scale = g.FontBaseSize * FontWindowScale; if (ParentWindow) scale *= ParentWindow->FontWindowScale; return scale; }
    float       TitleBarHeight() const  { ImGuiContext& g = *GImGui; return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : CalcFontSize() + g.Style.FramePadding.y * 2.0f; }
    ImRect      TitleBarRect() const    { return ImRect(centerPos, ImVec2(centerPos.x + SizeFull.x, centerPos.y + TitleBarHeight())); }
    float       MenuBarHeight() const   { ImGuiContext& g = *GImGui; return (Flags & ImGuiWindowFlags_MenuBar) ? DC.MenuBarOffset.y + CalcFontSize() + g.Style.FramePadding.y * 2.0f : 0.0f; }
    ImRect      MenuBarRect() const     { float y1 = centerPos.y + TitleBarHeight(); return ImRect(centerPos.x, y1, centerPos.x + SizeFull.x, y1 + MenuBarHeight()); }
};
