...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        float x1 = window->centerPos.x;
        float x2 = window->centerPos.x + window->Size.x;
...
void ImGui::EndMenuBar()
{
    ...
    PopClipRect();
    PopID();
    window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->centerPos.x; // Save horizontal position so next append can reuse it. This is kinda equivalent to a per-layer CursorPos.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            ...
            ImVec2 tb = (window->centerPos.x < child_menu_window->centerPos.x) ? next_window_rect.GetTL() : next_window_rect.GetTR();
            ImVec2 tc = (window->centerPos.x < child_menu_window->centerPos.x) ? next_window_rect.GetBL() : next_window_rect.GetBR();
            float extra = ImClamp(ImFabs(ta.x - tb.x) * 0.30f, ref_unit * 0.5f, ref_unit * 2.5f);   // add a bit of extra slack.
            ta.x += (window->centerPos.x < child_menu_window->centerPos.x) ? -0.5f : +0.5f;                     // to avoid numerical issues (FIXME: ??)
