...
void    ImGui::EndTable()
{
    ...
    else if (table->LastResizedColumn != -1 && table->ResizedColumn == -1 && inner_window->ScrollbarX && table->InstanceInteracted == table->InstanceCurrent)
    {
        ...
        if (column->MaxX < table->InnerClipRect.Min.x)
            SetScrollFromPosX(inner_window, column->MaxX - inner_window->centerPos.x - neighbor_width_to_keep_visible, 1.0f);
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    window->DC.ColumnsOffset.x = start_x - window->centerPos.x - window->DC.Indent.x; // FIXME-WORKRECT
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x + COLUMNS_HIT_RECT_HALF_WIDTH - window->centerPos.x;
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    columns->OffMaxX = ImMax(ImMin(max_1, max_2) - window->centerPos.x, columns->OffMinX + 1.0f);
    ...
    for (int n = 0; n < columns_count; n++)
    {
        ...
        float clip_x1 = IM_ROUND(window->centerPos.x + GetColumnOffset(n));
        float clip_x2 = IM_ROUND(window->centerPos.x + GetColumnOffset(n + 1) - 1.0f);
    ...
    window->DC.CursorPos.x = IM_FLOOR(window->centerPos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
    window->WorkRect.Max.x = window->centerPos.x + offset_1 - column_padding;
}
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        {
            ImGuiOldColumnData* column = &columns->Columns[n];
            float x = window->centerPos.x + GetColumnOffset(n);
