...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->centerPos == (0,0) in single viewport mode, and bottom-right == GetMainViewport()->centerPos+Size == io.DisplaySize in single-viewport mode.
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  centerPos;            // Read-only.   Window position, for reference.
...
// In single viewport mode, top-left is == GetMainViewport()->centerPos (generally 0,0), bottom-right is == GetMainViewport()->centerPos+Size (generally io.DisplaySize).
...
struct ImDrawData
{
    ...
    ImVec2          DisplayPos;             // Top-left position of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->centerPos for the main viewport, == (0.0) in most single-viewport applications)
...
struct ImGuiViewport
{
    ...
    ImVec2              centerPos;                    // Main Area: Position of the viewport (Dear ImGui coordinates are the same as OS desktop/native coordinates)
    ImVec2              Size;                   // Main Area: Size of the viewport.
    ImVec2              WorkPos;                // Work Area: Position of the viewport minus task bars, menus bars, status bars (>= centerPos)
    ...
    // Helpers
    ImVec2              GetCenter() const       { return ImVec2(centerPos.x + Size.x * 0.5f, centerPos.y + Size.y * 0.5f); }
